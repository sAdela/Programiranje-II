...
const char * Vrsta[5] = { "Seminarski", "Parcijalni1", "Parcijalni2", "Integralni", "Prakticni" };
...
struct ObavezeNaPredmetu {
	...
	void Ispis() {
		cout << Vrsta[*_vrstaObaveze] << " " << _ocjena << " " << _napomena;
...
const char * _kljucneRijeci[15] = { NULL }; /*da bi se odredjena obaveza na predmetu smatrala uspjesno izvrsenom,
											u napomeni (objekta ObavezeNaPredmetu) ne smije stajati niti jedna od rijeci
											koja se nalazi u nizu _kljucneRijeci*/
...
struct PolozeniPredmet {
		...
		for (int i = 0; i < _trenutnoIzvrsenihObaveza; i++) {
			...
			if (*_listaIzvrsenihObaveza[i]._vrstaObaveze == vrsta) {
				...
				for (int j = 0; j < 15; j++) {
					...
					if (_listaIzvrsenihObaveza[i]._ocjena != 5 && postojiKljucna == false)
						return false;

					postojiKljucna = false;
				}
			}
		}

		ObavezeNaPredmetu *nova = new ObavezeNaPredmetu[_trenutnoIzvrsenihObaveza + 1];

		for (int i = 0; i < _trenutnoIzvrsenihObaveza; i++) {

			nova[i].Unos(*_listaIzvrsenihObaveza[i]._vrstaObaveze, _listaIzvrsenihObaveza[i]._datumIzvrsenja, _listaIzvrsenihObaveza[i]._ocjena, _listaIzvrsenihObaveza[i]._napomena);

			_listaIzvrsenihObaveza[i].Dealociraj();

		}
		nova[_trenutnoIzvrsenihObaveza].Unos(vrsta, datum, ocjena, napomena);

		delete[] _listaIzvrsenihObaveza;
		_listaIzvrsenihObaveza = nova;
	...
	bool ProvjeriKljucne(const char* napomena) {
			
			for (int i = 0; i < 15; i++) 
				if (_kljucneRijeci[i] != nullptr) 
					if (strstr(napomena, _kljucneRijeci[i]) != nullptr)
					return true;
	...
	float FormirajKonacnuOcjenu() {
		...
		ocijenjena najmanje dva seminarska i jedan prakticni rad. Takodjer, datum polaganja drugog parcijalnog ispita ne smije biti raniji
		(manji) od datuma polaganja prvog parcijalnog ispita. Konacna ocjena ne smije biti formirana u slucaju da napomena bilo koje odbaveze
		...
		Ukoliko su ispunjeni svi uslovi, osim formiranja konacne ocjene, datum polaganja je potrebno postaviti
		...
		*/
		...
		for (int i = 0; i < _trenutnoIzvrsenihObaveza; i++) {
			...
			if (ProvjeriKljucne(_listaIzvrsenihObaveza[i]._napomena) == false) {
				if (_listaIzvrsenihObaveza[i]._ocjena > 5) {
					nizObaveza[*_listaIzvrsenihObaveza[i]._vrstaObaveze]++;
					nizOcjena[*_listaIzvrsenihObaveza[i]._vrstaObaveze] += _listaIzvrsenihObaveza[i]._ocjena;
				}
				if (nizObaveza[3] == 1) {
		...
		for (int i = 0; i < _trenutnoIzvrsenihObaveza; i++) {
			if (*_listaIzvrsenihObaveza[i]._vrstaObaveze == 1)
				prvi = _listaIzvrsenihObaveza[i]._datumIzvrsenja;
			else if (*_listaIzvrsenihObaveza[i]._vrstaObaveze == 2)
				drugi = _listaIzvrsenihObaveza[i]._datumIzvrsenja;
		}

		if (nizObaveza[1] == 1 && nizObaveza[2] == 1) {

			if (drugi.PretvoriUDane() - prvi.PretvoriUDane() < 0) {
				_konacnaOcjena = 5;
				return _konacnaOcjena;
			}

			if (nizObaveza[0] != 2 || nizObaveza[4] != 1) {
				_konacnaOcjena = 5;
				return _konacnaOcjena;
			}

			if (nizObaveza[0] == 2 || nizObaveza[4] == 1) {
				_konacnaOcjena = ((nizOcjena[1] + nizOcjena[2] + nizOcjena[4] + nizOcjena[0]) / 4.0) + 0.5;
			}
		}
		int najveca = 0;
		for (int i = 0; i < _trenutnoIzvrsenihObaveza; i++) {
			if (_listaIzvrsenihObaveza[i]._ocjena > najveca) {
				najveca = _listaIzvrsenihObaveza[i]._ocjena;
...
float PretragaRekurzivno(PolozeniPredmet predmet, VrstaObaveze vrsta, Datum OD, Datum DO, int brojObaveza, int prosjek, int brojObavezaZaProsjek=0) {

	if (predmet._trenutnoIzvrsenihObaveza == brojObaveza)
		return float(prosjek) / brojObavezaZaProsjek;

	else if (*predmet._listaIzvrsenihObaveza[brojObaveza]._vrstaObaveze == vrsta) {

		if (predmet._listaIzvrsenihObaveza[brojObaveza]._datumIzvrsenja.PretvoriUDane() > OD.PretvoriUDane() && predmet._listaIzvrsenihObaveza[brojObaveza]._datumIzvrsenja.PretvoriUDane() < DO.PretvoriUDane())
			return PretragaRekurzivno(predmet, vrsta, OD, DO, brojObaveza + 1, predmet._listaIzvrsenihObaveza[brojObaveza]._ocjena, brojObavezaZaProsjek + 1);

		else return PretragaRekurzivno(predmet, vrsta, OD, DO, brojObaveza + 1, prosjek, brojObavezaZaProsjek);
	}
...
int GetBrojObavezaIznadProsjeka(PolozeniPredmet predmet, float prosjek) {
	...
	/*Koristeci lambda-u, unutar funkcije GetBrojObavezaIznadProsjeka pronaci broj izvrsenih obaveza koje imaju istu
	ili vecu ocjenu od prosjecne*/

	auto iznadProsjeka = [predmet, prosjek]() {
		int brojac = 0;
...
void main() {
	...
	*/
	...
	///*Rekurzivna funkcija treba vratiti prosjecnu ocjenu uspjesno izvrsenih obaveze na predmetu odredjene vrste
	//u definisanom periodu. Izbjegavati koristenje globalnih varijabli*/
	...
	//datumIntegralni.Dealociraj(); //Nije unesen datum
	prII.Dealociraj();
	system("Pause>0");
